...
@pytest.fixture
def mock_groq_client_create():
    with patch("backend.services.groq_service.GroqService.client.chat.completions.create") as mock_create:
        yield mock_create
...
def test_generate_product_insights_success(groq_service, mock_groq_client_create):
    expected_insights = {
    ...
    mock_groq_client_create.return_value = MockCompletion(json.dumps(expected_insights))

    description = "A test product description"
    insights = groq_service.generate_product_insights(description)

    assert insights == expected_insights
    mock_groq_client_create.assert_called_once()

def test_generate_product_insights_invalid_json(groq_service, mock_groq_client_create):
    invalid_json_response = "This is not valid JSON"
    mock_groq_client_create.return_value = MockCompletion(invalid_json_response)
...
def test_compare_product_descriptions_success(groq_service, mock_groq_client_create):
    expected_comparison = {
        "comparison": {
    ...
    mock_groq_client_create.return_value = MockCompletion(json.dumps(expected_comparison))
...
def test_compare_product_descriptions_invalid_json(groq_service, mock_groq_client_create):
